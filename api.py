...
def load_resources():
    ...
    # Load model
    model_path = 'data/failure_predictor.pkl'
...
def get_engines():
    ...
    for engine_id in engines:
        ...
        # Simple failure prediction based on latest data
        has_failure_data = latest['failure'] == 1
        
        engine_list.append({
            'id': int(engine_id),
            'name': f'ENG-{engine_id:03d}',
            'status': 'at_risk' if has_failure_data else 'normal',
...
def get_fleet_stats():
    ...
    at_risk_engines = int(latest_per_engine['failure'].sum())
    healthy_engines = total_engines - at_risk_engines
